...
export const DEFAULT_EMAIL_NOTIFICATION_SETTINGS = {
  [EMAIL_NOTIFICATION_SETTINGS.JOB_NOTIFICATION_FREQUENCY]: 5, // Send after every 5 jobs
 ...
 */
export const getAllEmailNotificationSettings = async (): Promise<Record<string, any>> => {
  try {
    ...
    // Set defaults first
    Object.entries(DEFAULT_EMAIL_NOTIFICATION_SETTINGS).forEach(([key, value]) => {
 ...
 */
export const getEmailNotificationSetting = async (key: string): Promise<any> => {
  try {
    ...
    return DEFAULT_EMAIL_NOTIFICATION_SETTINGS[key as keyof typeof DEFAULT_EMAIL_NOTIFICATION_SETTINGS];
  } catch (error) {
  ...
  switch (key) {
    case EMAIL_NOTIFICATION_SETTINGS.JOB_NOTIFICATION_FREQUENCY:
      if (!Number.isInteger(value) || value < 1 || value > 50) {
        throw new Error("Job notification frequency must be an integer between 1 and 50");
      }
      break;
...
function getSettingDescription(key: string): string {
  switch (key) {
    case EMAIL_NOTIFICATION_SETTINGS.JOB_NOTIFICATION_FREQUENCY:
      return "Number of jobs after which to send notification emails to recruiters";
    case EMAIL_NOTIFICATION_SETTINGS.END_OF_DAY_NOTIFICATIONS:
      return "Whether to send end-of-day notification emails even if frequency threshold is not met";
    case EMAIL_NOTIFICATION_SETTINGS.END_OF_DAY_TIME:
...
export const areNotificationsEnabled = async (): Promise<boolean> => {
  try {
    const enabled = await getEmailNotificationSetting(EMAIL_NOTIFICATION_SETTINGS.NOTIFICATION_ENABLED);
...
export const getJobNotificationFrequency = async (): Promise<number> => {
  try {
    const frequency = await getEmailNotificationSetting(EMAIL_NOTIFICATION_SETTINGS.JOB_NOTIFICATION_FREQUENCY);
...
export const areEndOfDayNotificationsEnabled = async (): Promise<boolean> => {
  try {
    const enabled = await getEmailNotificationSetting(EMAIL_NOTIFICATION_SETTINGS.END_OF_DAY_NOTIFICATIONS);
    return Boolean(enabled);
  } catch (error) {
    console.error("Error checking if end-of-day notifications are enabled:", error);
    return true; // Default to enabled
  }
};
...
export const getEndOfDayTime = async (): Promise<string> => {
  try {
    const time = await getEmailNotificationSetting(EMAIL_NOTIFICATION_SETTINGS.END_OF_DAY_TIME);
