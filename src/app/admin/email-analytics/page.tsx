...
import { Loader2, Mail, TrendingUp, AlertCircle, CheckCircle, Clock } from 'lucide-react';

interface EmailAnalytics {
  dailyStats: Array<{
    date: string;
    usageLimit: { sent: number; failed: number; pending: number; recipients: number };
    eod: { sent: number; failed: number; pending: number; recipients: number };
    total: { sent: number; failed: number; pending: number; recipients: number };
  }>;
  overallStats: {
    totalEmails: number;
...
export default function EmailAnalyticsPage() {
  ...
  const { overallStats, successRateByType, recentFailures, dailyStats } = analytics;
  const successRate = overallStats.totalEmails > 0 
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <Button onClick={() => fetchAnalytics(selectedDays)} variant="outline">
            Refresh
          </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{overallStats.totalEmails}</div>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Usage Limit Emails</CardTitle>
            <CheckCircle className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{overallStats.usageLimitEmails}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">{overallStats.eodEmails}</div>
            <p className="text-xs text-muted-foreground">
              ...
              <div key={type._id} className="flex items-center justify-between p-4 border rounded-lg">
                <div>
                  <h3 className="font-medium">
                    {type._id === 'job_batch' ? 'Usage Limit Emails' : 'End-of-Day Emails'}
                  </h3>
                ...
                <div className="flex items-center gap-4">
                  <div className="text-right">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {type.successful} sent, {type.failed} failed, {type.pending} pending
                    </div>
                  </div>
          ...
          <CardContent>
            <div className="space-y-3">
              {recentFailures.map((failure) => (
                <div key={failure._id} className="flex items-center justify-between p-3 border border-red-200 rounded-lg bg-red-50">
                  <div>
                    ...
                    <p className="text-sm mt-1">
                      {failure.recipientCount} recipients • {failure.retryCount} retries
                    </p>
