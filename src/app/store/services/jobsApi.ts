...
export const jobsApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    updateJob: builder.mutation<IJob, { id: string; job: Partial<JobFormData> }>({
      query: ({ id, job }) => ({
        url: `/${id}`,
        method: 'PUT',
        body: job,
    ...
    addScreeningQuestion: builder.mutation<
      { success: boolean },
      { jobId: string; question: string; questionType: string; required: boolean }
    >({
      query: ({ jobId, ...rest }) => ({
        url: `/${jobId}/questions`,
        method: 'POST',
    ...
    updateScreeningQuestion: builder.mutation<
      { success: boolean },
      { jobId: string; questionId: string; question: string; questionType: string; required: boolean }
    >({
