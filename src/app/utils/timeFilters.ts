...
export const filterJobStatusData = (jobs: any[], timeFrame: string): any[] => {
  if (timeFrame === "all" || !jobs) {
    return [
      ...
      {
        name: "Closed",
        value: jobs?.filter((job) => job.status === "CLOSED").length || 0,
      },
      {
        name: "Draft",
        value: jobs?.filter((job) => job.status === "DRAFT").length || 0,
      },
      {
        name: "Paused",
        value: jobs?.filter((job) => job.status === "PAUSED").length || 0,
      },
    ];
  }
  ...
  return [
    {
      name: "Active",
      value: filteredJobs.filter((job) => job.status === "ACTIVE").length,
    },
    {
      name: "Closed",
      value: filteredJobs.filter((job) => job.status === "CLOSED").length,
    },
    {
      name: "Draft",
      value: filteredJobs.filter((job) => job.status === "DRAFT").length,
    },
    {
      name: "Paused",
      value: filteredJobs.filter((job) => job.status === "PAUSED").length,
    },
  ];
};
 ...
 */
export const filterResumeStatusData = (
  resumes: any[],
  timeFrame: string
): any[] => {
    ...
    return [
      {
        name: "Submitted",
        value:
          resumes?.filter((resume) => resume.status === "SUBMITTED").length ||
          0,
      },
      {
        name: "Shortlisted",
        value:
          resumes?.filter((resume) => resume.status === "SHORTLISTED").length ||
          0,
      },
      {
        name: "Rejected",
        value:
          resumes?.filter((resume) => resume.status === "REJECTED").length || 0,
      },
      {
        name: "Hired",
        value:
          resumes?.filter((resume) => resume.status === "HIRED").length || 0,
      },
      {
        name: "Duplicate",
        value:
          resumes?.filter((resume) => resume.status === "DUPLICATE").length ||
          0,
      },
    ];
  }
  ...
  return [
    {
      name: "Submitted",
      value: filteredResumes.filter((resume) => resume.status === "SUBMITTED")
        .length,
    },
    {
      name: "Shortlisted",
      value: filteredResumes.filter((resume) => resume.status === "SHORTLISTED")
        .length,
    },
    {
      name: "Rejected",
      value: filteredResumes.filter((resume) => resume.status === "REJECTED")
        .length,
    },
    {
      name: "Hired",
      value: filteredResumes.filter((resume) => resume.status === "HIRED")
        .length,
    },
    {
      name: "Duplicate",
      value: filteredResumes.filter((resume) => resume.status === "DUPLICATE")
        .length,
    },
  ];
};
 ...
 */
export const filterUsersByRoleData = (
  statsData: any,
  recentUsers: any[],
  timeFrame: string
): any[] => {
 ...
 */
export const getFilteredStatCards = (
  statsData: any,
  jobs: any[],
  resumes: any[],
  timeFrame: string
  ...
  const filteredJobs =
    timeFrame !== "all" ? filterDataByTime(jobs || [], timeFrame) : jobs;
  const filteredResumes =
    timeFrame !== "all" ? filterDataByTime(resumes || [], timeFrame) : resumes;

  // Calculate hiring rate
  const hiredCount =
    filteredResumes?.filter((resume) => resume.status === "HIRED").length || 0;
  const hiringRate =
  ...
  return [
    ...
    {
      title: "Internal Team",
      value: statsData?.stats.users.byRole.internal || 0,
      bgColor: "bg-teal-500",
      icon: "User",
      link: "/dashboard/admin/users?role=INTERNAL",
    },
    {
      title: "Active Jobs",
