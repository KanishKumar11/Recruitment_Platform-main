...
export async function GET(request: NextRequest) {
  try {
    ...
    const failedCount = await EmailNotification.countDocuments({
      status: "failed",
    });
    const pendingCount = await EmailNotification.countDocuments({
      status: "pending",
    });
    const sentCount = await EmailNotification.countDocuments({
      status: "sent",
    });
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (testType === "basic") {
      ...
      testResult = await sendOTPEmail(testEmail, "Test User", "TEST123");
    }
