...
import { authenticateRequest, authorizeRoles, unauthorized, forbidden, JwtPayload } from './../../../lib/auth';
...
function validateCommissionData(commission: any) {
  ...
  if (type === 'percentage') {
    if (typeof originalPercentage !== 'number' || originalPercentage <= 0 || originalPercentage > 50) {
      return { isValid: false, error: 'Commission percentage must be between 1 and 50' };
    }
  }

  if (type === 'fixed') {
    if (typeof fixedAmount !== 'number' || fixedAmount <= 0) {
      return { isValid: false, error: 'Fixed commission amount must be greater than 0' };
    }
  }
...
async function hasAccessToJob(userData: JwtPayload, job: { status: string; postedBy: { toString: () => any; }; }) {
...
export async function GET(
  ...
  try {
    ...
    const jobPoster = await User.findById(job.postedBy).select('name companyName');
    
    if (jobPoster) {
      // For recruiters, show the company name
      if (userData.role === UserRole.RECRUITER || userData.role === UserRole.ADMIN || userData.role === UserRole.INTERNAL) {
      ...
      if (job.postedBy.toString() !== userData.userId && userData.role === UserRole.COMPANY) {
...
export async function PUT(
  ...
  try {
    ...
    if (updatedJob && updatedJob.screeningQuestions && updatedJob.screeningQuestions.length > 0) {
