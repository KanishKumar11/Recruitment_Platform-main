...
export async function POST(req: NextRequest) {
  try {
    ...
    if (role === UserRole.COMPANY) {
      if (!companyName || !companySize || !designation) {
        ...
        return NextResponse.json(
          {
            error:
              "Company name, company size, and designation are required for company registration",
          },
          { status: 400 }
        );
      }
      ...
      if (personalEmailDomains.includes(domain)) {
        console.log(
          `Personal email domain not allowed for company registration: ${domain}`
        );
        return NextResponse.json(
          {
            error:
              "Company email required. Personal email domains (gmail.com, yahoo.com, etc.) are not allowed for company registration.",
          },
          { status: 400 }
        );
      }
    ...
    if (role === UserRole.RECRUITER && recruitmentFirmName) {
      ...
      console.log(
        `Adding recruitment firm name to userData: ${userData.recruitmentFirmName}`
      );
    }
    ...
    console.log(
      "OTP verification record saved with userData:",
